...
import { useRemoverProblemaDaCompeticao } from '../competicao.hooks'
import ProblemaCard from '../../problema/components/ProblemaCard'

interface CompeticaoProblemasListProps {
  competicaoId: number
  isProfessor: boolean
}

export default function CompeticaoProblemasList({ competicaoId, isProfessor }: CompeticaoProblemasListProps) {
  const navigate = useNavigate()
  const { data: paginaProblemas, isLoading, isError } = useQuery({
    queryKey: problemaKeys.list({ competicaoId, pagina: 0, tamanho: 100 }),
    queryFn: () => problemaService.listarProblemas({ competicaoId, pagina: 0, tamanho: 100 }),
  })
  ...
  if (isError || !paginaProblemas) {
    return (
      <div className="glass-card flex flex-col items-center justify-center gap-4 py-16 text-center">
        ...
        <p className="font-bold text-spif-text">Erro ao carregar problemas da competição.</p>
      </div>
    )
  }
      ...
      <div className="glass-card border-dashed flex flex-col items-center justify-center py-24 text-center gap-6">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-spif-secondary">
            {isProfessor
              ? 'Vá na aba "Adicionar" para vincular problemas a esta competição.'
              : 'Esta competição ainda não possui problemas cadastrados.'}
          </p>
        </div>
      </div>
    )
    ...
    <div className="grid gap-6 grid-cols-1 md:grid-cols-2 xl:grid-cols-3 animate-in fade-in duration-700">
      {problemas.map((p) => (
        <ProblemaCard
          ...
          onClick={() => navigate(`/competicoes/${competicaoId}/problemas/${p.id}`)}
        />
      ))}
    </div>
  )
