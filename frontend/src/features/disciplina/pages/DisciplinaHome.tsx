...
import MinhasDisciplinasPage from './MinhasDisciplinasPage'
import ConvitesDisciplinasPage from './ConvitesDisciplinasPage'
import TodasDisciplinasPage from './TodasDisciplinasPage'
import CriarDisciplinaForm from '../components/CriarDisciplinaForm'

type Tab = 'MINHAS' | 'CONVITES' | 'TODAS' | 'CRIAR'
...
export default function DisciplinaHome() {
  ...
  const tabs = [
    ...
    { key: 'CONVITES', label: 'Convites', show: !isProfessor },
    { key: 'TODAS', label: 'Todas', show: isProfessor },
    { key: 'CRIAR', label: 'Nova Disciplina', show: isProfessor }
    ...
    <div className="min-h-[calc(100vh-80px)] bg-spif-bg text-spif-text fade-out duration-300">
      <div className="max-w-[1600px] mx-auto px-8 md:px-12 py-12">
        ...
        <main className="animate-in fade-out slide-in-from-bottom-2 duration-300">
          <div className="mt-4">
            {activeTab === 'MINHAS' && <MinhasDisciplinasPage />}
            {activeTab === 'CONVITES' && !isProfessor && <ConvitesDisciplinasPage onSuccess={() => setActiveTab('MINHAS')} />}
            {activeTab === 'TODAS' && isProfessor && <TodasDisciplinasPage onSuccess={() => setActiveTab('MINHAS')} />}
            {activeTab === 'CRIAR' && isProfessor && (
              <div className="glass-card p-8">
                <CriarDisciplinaForm onSuccess={() => setActiveTab('TODAS')}  />
              </div>
            )}
          </div>
        </main>
